...
SELF_EXE = target/release/chat-server

.PHONY: all
all: $(SELF_EXE) chat-server.manifest
ifeq ($(SGX),1)
all: chat-server.manifest.sgx chat-server.sig
endif
...
chat-server.manifest: chat-server.manifest.template
	gramine-manifest \
...
chat-server.manifest.sgx chat-server.sig: sgx_sign
	@:

.INTERMEDIATE: sgx_sign
sgx_sign: chat-server.manifest $(SELF_EXE)
	gramine-sgx-sign \
		--manifest $< \
		--output $<.sgx
